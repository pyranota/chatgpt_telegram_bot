...
class ChatGPT:
        ...
        while answer is None:
            try:
                if self.model in {
                    ...
                    r = await aclient.chat.completions.create(model=self.model, messages=messages, **OPENAI_COMPLETION_OPTIONS)
                    ...
                    r = await aclient.completions.create(engine=self.model, prompt=prompt, **OPENAI_COMPLETION_OPTIONS)
    ...
    async def send_message_stream(
        ...
        # raise ValueError(f"Chat mode {dialog_messages} is not supported")
        if chat_mode not in config.chat_modes.keys():
            raise ValueError(f"Chat mode {chat_mode} is not supported")
        ...
        while answer is None:
            try:
                if self.model in {
                    ...
                    # raise ValueError(f"Chat mode {messages} is not supported")
                    ...
                    async for r_item in r_gen:
                        ...
                        if delta.content != None:
                            ...
                            logger.error(f"in: {n_input_tokens}\n out: {n_output_tokens}")
